...
#+BEGIN_SRC ruby ::results value raw :exports none :var caption="ほほげほげ" :var figPicsPath="hoge{1,2}.png" :var dirPath="./"
    require 'pathname'
...
- 粘性
  - 水平方向の渦粘性係数 $A_h$ , 渦拡散係数 $K_h$  の設定
    - $A_h, K_h$ の大きさは, 次の各ケース設定を参照. 
  - 鉛直方向の渦粘性係数 $A_v$ , 渦拡散係数 $K_v$ の設定
    - 海面流速が \cite{marshall2007mean} と同程度になるように, $A_v=1.0 \times 10^{-2}$ [m^2 s^{-1}] にとる. 
    - $K_v$ の大きさは, 次の各ケース設定を参照. 
...
- 海面
  ...
  #+CAPTION: 海面応力(\tau_{zx})の南北分布[ N/m^{2} ].   
  | [[./data/common/windStressLon.jpg][file:./expdata_inhomoFluid/common/windStressLon_thumb.png]] |
  ...
  - 熱的条件: 
    ...
    \[
        \left(\dfrac{\partial\theta}{\partial t}\right)_{\rm relax} = - \dfrac{1}{\tau} (\theta - \theta_{\rm relax}), \;\;\;
        \left(\dfrac{\partial S}{\partial t}\right)_{\rm relax} = - \dfrac{1}{\tau} (S - S_{\rm relax})  \;\;\;\;\;\; {\rm at} \; z=0. 
    \]   

  #+CAPTION: 海面温度(\theta_{relax}) [K], 海面塩分(S_{relax}) [psu] の南北分布.   
  | [[./data/common/windStressLon.jpg][file:./expdata_inhomoFluid/common/y_PTempRelax_thumb.png]]  |  [[./data/common/windStressLon.jpg][file:./expdata_inhomoFluid/common/y_SaltRelax_thumb.png]]     |
...
- 海底
  ...
  - 熱的条件: 断熱条件(境界を横切るフラックスはゼロ)
    ...
    \[
        K_v \dfrac{\partial \theta}{\partial z} = 0, \;\;\;
...
- 温位・塩分場: 20 世紀の気候の数値実験の結果[fn:init_dataset]を時間・水平平均したものを, 初期の鉛直分布として与える(水平一様). 
  ...
  #+CAPTION: 温位[K], 塩分[psu] の初期の鉛直分布(水平方向は一様). 
  | [[./data/common/windStressLon.jpg][file:./expdata_inhomoFluid/common/z_PTempInit_thumb.png]]   |  [[./data/common/windStressLon.jpg][file:./expdata_inhomoFluid/common/z_SaltInit_thumb.png]]      |
...
- 各実験の設定ファイル
  ...
  #+CALL: create_link("Ah1e4Prh1Prv1(標準実験),Ah1e3Prh1Prv1,Ah1e5Prh1Prv1", "exp_Ah1e{4,3,5}Prh1Prv1/config.nml") :results value raw :exports results
